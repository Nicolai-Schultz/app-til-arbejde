...
if selected == "Kreditmaksudregner":
        ...
        if(kredit_input < beløb_input):
                st.success(f"Kunden skal indbetale {udregning} for at deres abonnementer kan genåbnes.")
                st.write(f"Kopier følgende beskrivelse med udregning ind i loggen: Kunde ringer angående spærring, hvilket er grundet overskridelse af kreditmaks. Informerer at kunde skal indbetale ({beløb_input} kr - ({kredit_input} kr * 0.8) + 1 kr = {udregning} kr), således deres abonnementer genåbnes.")
